...
def create_task(path: ProjectPath, body: CreateTaskBody):
    ...
    task = Task(
        ...
        assignee_id=validate_assignee(body.assignee_id, path.project_id)
        if body.assignee_id
        else None,
...
def update_task(path: TaskPath, body: UpdateTaskBody):
    ...
    if "assignee_id" in data:
        task.assignee_id = (
            validate_assignee(body.assignee_id, path.project_id)
            if body.assignee_id
            else None
        )
    if "due_date" in data:
        task.due_date = (
            validate_date(body.due_date, "due_date") if body.due_date else None
        )
    db.session.commit()
    return jsonify(task.to_dict()), 200
